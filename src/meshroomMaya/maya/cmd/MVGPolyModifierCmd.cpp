#include "meshroomMaya/maya/cmd/MVGPolyModifierCmd.hpp"
...
MVGPolyModifierCmd::MVGPolyModifierCmd()
{
...
MStatus MVGPolyModifierCmd::initModifierNode(MObject /* modifierNode */)
{
    // Description:
    // Override this method in a derived class to set input attributes on the modifier
    // node. If not overidden, the modifier node will remain in it's default state upon
    // construction.
    // The argument 'MObject modifierNode', is not used by this base class implementation.However,
    // it may be used by derived classes.To avoid compiler warnings of unreferenced parameters, we
    // comment out the parameter name.
    return MS::kSuccess;
}

MStatus MVGPolyModifierCmd::directModifier(MObject /* mesh */)
{
    // Description:
    // Override this method in a derived class to provide an implementation for directly modifying
    // the mesh (writing on the mesh itself). This method is
    // only called in the case where history does not exist and history is turned off (ie. DG
    // operations are not desirable).
    // The argument 'MObject mesh', is not used by this base class implementation.
    // However, it may be used by derived classes. To avoid compiler warnings of unreferenced
    // parameters, we comment out the parameter name.
    return MS::kSuccess;
}

MStatus MVGPolyModifierCmd::doModifyPoly()
{
...
MStatus MVGPolyModifierCmd::redoModifyPoly()
{
    MStatus status = MS::kSuccess;
    if(!fHasHistory && !fHasRecordHistory)
    {
        MObject meshNode = fDagPath.node();
        // Call the directModifier - No need to pre-process the mesh data again since we already
        // have it.
        status = directModifier(meshNode);
    }
    else
...
MStatus MVGPolyModifierCmd::undoModifyPoly()
{
    MStatus status = MS::kSuccess;
    if(!fHasHistory && !fHasRecordHistory)
    {
    ...
    {
        fDGModifier.undoIt();
        // undoCachedMesh must be called before undoTweakProcessing because undoCachedMesh copies
        // the original mesh *without* tweaks back onto the existing mesh. Any changes done before
...
bool MVGPolyModifierCmd::isCommandDataValid()
{
    bool valid = true;
...
void MVGPolyModifierCmd::collectNodeState()
{
    MStatus status;
    ...
    // Tweaks exist only if the multi "pnts" attribute contains plugs which contain non-zero tweak
    // values. Use false, until proven true search algorithm.
    fHasTweaks = false;
    MPlug tweakPlug = depNodeFn.findPlug("pnts");
    if(!tweakPlug.isNull())
    {
        // ASSERT: tweakPlug should be an array plug!
        // MAssert((tweakPlug.isArray()), "tweakPlug.isArray() -- tweakPlug is not an array plug");
        MPlug tweak;
        ...
        for(i = 0; i < numElements; i++)
        {
            ...
            if(status == MS::kSuccess && !tweak.isNull())
            {
                ...
                if(0 != tweakData.x || 0 != tweakData.y || 0 != tweakData.z)
                {
                    fHasTweaks = true;
                    break;
                }
...
MStatus MVGPolyModifierCmd::createModifierNode(MObject& modifierNode)
{
    MStatus status = MS::kFailure;
    if(fModifierNodeTypeInitialized || fModifierNodeNameInitialized)
    {
...
MStatus MVGPolyModifierCmd::processMeshNode(modifyPolyData& data)
{
    MStatus status = MS::kSuccess;
    // Declare our function sets. Use MFnDagNode here so we can retrieve the parent transform.
...
MStatus MVGPolyModifierCmd::processUpstreamNode(modifyPolyData& data)
{
    MStatus status = MS::kSuccess;
    // Declare our function sets - Although dagNodeFn derives from depNodeFn, we need both since
    // dagNodeFn can only refer to DAG objects.
    // We will use depNodeFn for all times other when dealing with the DAG.
    MFnDependencyNode depNodeFn;
    MFnDagNode dagNodeFn;
    // Use the selected node's plug connections to find the upstream plug.
    // Since we are looking at the selected node's inMesh attribute, it will always have only one
    // connection coming in if it has history, and none otherwise.
    // If there is no history, copy the selected node and place it ahead of the modifierNode as the
    // new upstream node so that the modifierNode has an input mesh to operate on.
    MPlugArray tempPlugArray;
    if(fHasHistory)
    {
        // Since we have history, look for what connections exist on the meshNode "inMesh" plug.
        // "inMesh" plugs should only ever have one connection.
        ...
        // MStatusAssert((tempPlugArray.length() == 1), "tempPlugArray.length() == 1 "
        //                                              "-- 0 or >1 connections on "
        //                                              "meshNodeShape.inMesh");
        data.upstreamNodeSrcPlug = tempPlugArray[0];
        // Construction history only deals with shapes, so we can grab the upstreamNodeShape off of
        // the source plug.
        ...
        // Disconnect the upstream node and the selected node, so we can replace them with our own
        // connections below.
        fDGModifier.disconnect(data.upstreamNodeSrcPlug, data.meshNodeDestPlug);
    }
    else // No History (!fHasHistory)
    {
        // Use the DAG node function set to duplicate the shape of the meshNode.
        // The duplicate method will return an MObject handle to the transform of the duplicated
        // shape, so traverse the dag to locate the shape. Store this duplicate shape as our
        // "upstream" node to drive the input for the modifierNode.
        dagNodeFn.setObject(data.meshNodeShape);
        ...
        // MStatusAssert((0 < dagNodeFn.childCount()), "0 < dagNodeFn.childCount() -- "
        //                                             "Duplicate meshNode transform "
        //                                             "has no shape.");
        ...
        // Note: This reparent must be performed before the deleteNode() is called. See
        // MVGPolyModifierCmd.h (see definition of fDagModifier) for more details.
        ...
        // Note: This deleteNode must be performed after the reparentNode() method is completed. See
        // MVGPolyModifierCmd.h (see definition of fDagModifier) for details.
        status = fDagModifier.doIt();
        // MCheckStatus(status, "fDagModifier.doIt()");
        CHECK_RETURN_STATUS(status)
...
MStatus MVGPolyModifierCmd::processModifierNode(MObject modifierNode, modifyPolyData& data)
{
    MStatus status = MS::kSuccess;
    MFnDependencyNode depNodeFn(modifierNode);
...
MStatus MVGPolyModifierCmd::processTweaks(modifyPolyData& data)
{
    MStatus status = MS::kSuccess;
    ...
    if(fHasTweaks)
    {
        ...
        for(i = 0; i < numElements; i++)
        {
            // MPlug::numElements() only returns the number of physical elements in the array plug.
            // Thus we must use elementByPhysical index when using the index i.
            tweak = meshTweakPlug.elementByPhysicalIndex(i);
            // If the method fails, the element is NULL. Only append the index if it is a valid
            // plug.
            if(!tweak.isNull())
            {
                ...
                // Parse down to the deepest level of the plug tree and check for connections - look
                // at the child nodes of the element plugs.
                // If any connections are found, record the connection and disconnect it.
        ...
        for(i = 0; i < numTweaks; i++)
        {
            ...
            for(j = 0; j < numChildren; j++)
            {
                ...
                {
                    for(k = 0; k < (unsigned)tweakSrcConnectionCountArray[i * numChildren + j]; k++)
                    {
                        fDGModifier.connect(tweakChild, tweakSrcConnectionPlugArray[srcOffset]);
                        srcOffset++;
                    }
                }
                ...
                {
                    fDGModifier.connect(tweakDstConnectionPlugArray[dstOffset], tweakChild);
                    dstOffset++;
                }
            }
        }
...
MStatus MVGPolyModifierCmd::connectNodes(MObject modifierNode)
{
    MStatus status;
    // Declare our internal processing data structure (see MVGPolyModifierCmd.h for definition)
    modifyPolyData data;
...
MStatus MVGPolyModifierCmd::cacheMeshData()
{
    MStatus status = MS::kSuccess;
    MFnDependencyNode depNodeFn;
    MFnDagNode dagNodeFn;
...
MStatus MVGPolyModifierCmd::cacheMeshTweaks()
{
    MStatus status = MS::kSuccess;
    // Clear tweak undo information (to be rebuilt)
    fTweakIndexArray.clear();
...
MStatus MVGPolyModifierCmd::undoCachedMesh()
{
    MStatus status;
    // Only need to restore the cached mesh if there was no history. Also check to make sure that we
    // are in the record history state.
    ...
    if(!fHasHistory)
    {
        ...
        // MCheckStatus(status, "Could not retrieve outMesh");
        CHECK_RETURN_STATUS(status)
        // For the case with tweaks, we cannot write the mesh directly back onto the cachedInMesh,
        // since the shape can have out of date information from the
        // cachedInMesh, thus we temporarily connect the duplicate mesh shape to the mesh shape and
        // force a DG evaluation.
        // For the case without tweaks, we can simply write onto the outMesh, since the shape relies
        // solely on an outMesh when there is no history nor tweaks.
        if(fHasTweaks)
        {
...
MStatus MVGPolyModifierCmd::undoTweakProcessing()
{
    MStatus status = MS::kSuccess;
    if(fHasTweaks)
    {
        ...
        // In the case of no history, the duplicate node shape will be disconnected on undo so,
        // there is no need to undo the tweak processing on it.
    }
    return status;
}

MStatus MVGPolyModifierCmd::undoDirectModifier()
{
    MStatus status;
    MFnDependencyNode depNodeFn;
    MFnDagNode dagNodeFn;
    MObject meshNode = fDagPath.node();
    depNodeFn.setObject(meshNode);
    // For the case with tweaks, we cannot write the mesh directly back onto the cachedInMesh, since
    // the shape can have out of date information from the cachedInMesh. Thus we temporarily create
    // an duplicate mesh, place our old mesh on the outMesh attribute of our duplicate mesh, connect
    // the duplicate mesh shape to the mesh shape, and force a DG evaluation.
    // For the case without tweaks, we can simply write onto the outMesh, since the shape relies
    // solely on an outMesh when there is no history nor tweaks.
    if(fHasTweaks)
    {
        ...
        // The dagNodeFn::duplicate() returns a transform, but we need a shape so retrieve the DAG
        // path and extend it to the shape.
        MDagPath dupMeshDagPath;
        MDagPath::getAPathTo(dupMeshNode, dupMeshDagPath);
        dupMeshDagPath.extendToShape();
        // Retrieve the outMesh of the duplicate mesh and set our mesh data back on it.
        depNodeFn.setObject(dupMeshDagPath.node());
    ...
    {
        // Restore the original mesh by writing the old mesh data (fMeshData) back onto the outMesh
        // of our meshNode
        depNodeFn.setObject(meshNode);
...
MStatus MVGPolyModifierCmd::getFloat3PlugValue(MPlug plug, MFloatVector& value)
{
...
MStatus MVGPolyModifierCmd::getFloat3asMObject(MFloatVector value, MObject& object)
{
