...
#include "meshroomMaya/maya/context/MVGDrawUtil.hpp"
...
#include "meshroomMaya/qt/MVGUserLog.hpp"
                              ...
                              M3dView::DisplayStatus dispStatus)
{
    ...
    { // 2D drawing
        ...
        // Draw in MeshroomMaya viewports
        if(!isMVGView)
        {
            MVGDrawUtil::end2DDrawing();
            glDisable(GL_BLEND);
        ...
        // Draw in active MeshroomMaya viewport
        if(!isActiveView)
        {
                                     ...
                                     MPoint& triangulatedWSPoint)
{
    ...
    if(blindData.size() < 2)
        return false;
    MVGGeometryUtil::triangulatePoint(blindData, triangulatedWSPoint);
    return true;
}
