 ...
 */
bool binaryFiletoCameraOpenMVG( const std::string& sPathCameraBin,
 ...
 */
bool cameraOpenMVGtoBinaryFile( const std::string& sPathCameraBin,
...
struct CameraOpenMVG
{
  openMVG::PinholeCamera camera;
  std::string name;
  std::string imagePlaneName;
  
  CameraOpenMVG( const openMVG::PinholeCamera& camera,
                 const std::string& name,
                 const std::string imagePlaneName )
  {
    this->name = name;
    this->camera = camera;
    this->imagePlaneName = imagePlaneName;
  }
};
 ...
 */
bool readCameras( const std::string& pathViewTxt,
  ...
  while(fileView.good())
  {
    ...
    if (!temp.empty())
    {
      ...
      vec_cameraOpenMVG.push_back( CameraOpenMVG( camera, 
                                                  ...
                                                  stlplus::create_filespec( sDirectoryImagePlane, stlplus::basename_part( sImageName ) + "_of", stlplus::extension_part( sImageName ) ) ) );
    }
    temp.clear();
  } 
  return true;
