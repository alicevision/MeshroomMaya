...
bool Ply::open(const string_type& path, const ElementList& elements, const ObjInfoList& obj_infos,
               const CommentList& comments, Format format)
{
    _comments.clear();
    _obj_infos.clear();
    _elements.clear();

    if(format != PLY_FORMAT_ASC && format != PLY_FORMAT_BIN_BE && format != PLY_FORMAT_BIN_LE &&
    ...
    for(ElementsIterator it = elements.begin(); it != elements.end(); ++it)
        if(it->num_properties() == 0)
        {
            std::cerr << "Ply: element \"" << it->name() << "\" has no property" << std::endl;
            return false;
        }
    ...
    if(!_os.is_open())
    {
        std::cerr << "Ply: can't open file \"" << path << "\"" << std::endl;
        return false;
    }
    ...
    _os << "ply" << std::endl << "format " << format_enum_to_string(_format) << ' ' << major << '.'
        << minor << std::endl;

    for(CommentsIterator it = comments_begin(); it != comments_end(); ++it)
        _os << "comment " << *it << std::endl;

    for(ObjInfosIterator it = obj_infos_begin(); it != obj_infos_end(); ++it)
        _os << "obj_info " << *it << std::endl;

    for(ElementsIterator it = elements_begin(); it != elements_end(); ++it)
    {
        _os << "element " << it->name() << ' ' << it->count() << std::endl;

        for(PropertiesIterator it2 = it->properties_begin(); it2 != it->properties_end(); ++it2)
        {
...
bool Ply::open(const string_type& path)
{
    ...
    while(getline(_is, line_str))
    {
        ...
        if(!(iss >> token))
        {
            std::cerr << "Ply: can't extract first token on line " << (line_num + 1) << std::endl;
            _is.close();
            return false;
        }
        ...
        if(line_num == 0)
        {
            if(token != "ply")
            {
                std::cerr << "Ply: invalid first line in header" << std::endl;
                _is.close();
                return false;
            }

            if(iss >> token)
                std::cerr << "Ply: skipping garbage in header" << std::endl;
        ...
        {
            std::cerr << "Ply: unsupported token \"" << token << "\" on line " << (line_num + 1)
                      << std::endl;
            _is.close();
            return false;
        }
...
bool Ply::_skip_ASC(const Element& element)
{
    ...
    for(size_type i = 0; i != count; ++i)
        for(PropertiesIterator it = element.properties_begin(); it != element.properties_end();
            ++it)
        {
            ...
            if(!_skip_ASC(*it))
            {
                std::cerr << "Ply: error while skipping property \"" << it->name()
                          << "\" of element \"" << element.name() << "\" " << (i + 1) << std::endl;
                return false;
            }
...
bool Ply::_check_last_element() const
{
    ...
    {
        std::cerr << "Ply: element \"" << element.name() << "\" has no property" << std::endl;
        return false;
    }
    else
        return true;
...
bool Ply::_parse_format(std::istringstream& iss, size_type line_num)
{
    if(line_num != 1)
    {
        std::cerr << "Ply: unexpected format token on line " << (line_num + 1) << std::endl;
        return false;
    }
    ...
    if(!(iss >> token))
    {
        std::cerr << "Ply: can't extract format token on line " << (line_num + 1) << std::endl;
        return false;
    }
    ...
    if(format == PLY_FORMAT_UNKNOWN)
    {
        std::cerr << "Ply: unsupported format on line " << (line_num + 1) << std::endl;
        return false;
    }

    if((format == PLY_FORMAT_BIN_BE || format == PLY_FORMAT_BIN_LE) &&
    ...
    if(!(iss >> version))
    {
        std::cerr << "Ply: can't extract version token on line " << (line_num + 1) << std::endl;
        return false;
    }

    if(iss >> token)
    {
        std::cerr << "Ply: unexpected token \"" << token << "\" on line " << (line_num + 1)
                  << std::endl;
        return false;
    }
    ...
    if(version != 1.0f)
    {
        std::cerr << "Ply: unsupported version on line " << (line_num + 1) << std::endl;
        return false;
    }
...
bool Ply::_parse_element(std::istringstream& iss, size_type line_num)
{
    ...
    if(!(iss >> name))
    {
        std::cerr << "Ply: can't extract name token on line " << (line_num + 1) << std::endl;
        return false;
    }

    if(has_element(name))
    {
        std::cerr << "Ply: already specified element on line " << (line_num + 1) << std::endl;
        return false;
    }
    ...
    if(!(iss >> count))
    {
        std::cerr << "Ply: can't extract count token on line " << (line_num + 1) << std::endl;
        return false;
    }
    ...
    if(iss >> token)
    {
        std::cout << "Ply: unexpected token \"" << token << "\" on line " << (line_num + 1)
                  << std::endl;
        return false;
    }
...
bool Ply::_parse_property(std::istringstream& iss, size_type line_num)
{
    if(num_elements() == 0)
    {
        std::cerr << "Ply: property specified w/o element on line " << (line_num + 1) << std::endl;
        return false;
    }
    ...
    if(!(iss >> token))
    {
        std::cerr << "Ply: can't extract type token on line " << (line_num + 1) << std::endl;
        return false;
    }
    ...
    {
        std::cerr << "Ply: unsupported type on line " << (line_num + 1) << std::endl;
        return false;
    }
    ...
    if(type == PLY_TYPE_LIST)
    {
        ...
        {
            std::cerr << "Ply: unsupported count type on line " << (line_num + 1) << std::endl;
            return false;
        }
        else if( // count_type == PLY_TYPE_CHAR
            // || count_type == PLY_TYPE_SHORT
            // || count_type == PLY_TYPE_INT
            count_type == PLY_TYPE_FLOAT || count_type == PLY_TYPE_DOUBLE ||
            count_type == PLY_TYPE_LIST)
        {
            std::cerr << "Ply: invalid count type (" << type_enum_to_string(count_type)
                      << ") on line " << (line_num + 1) << std::endl;
            return false;
        }
        ...
        {
            std::cerr << "Ply: unsupported value type on line " << (line_num + 1) << std::endl;
            return false;
        }
        else if(value_type == PLY_TYPE_LIST)
        {
            std::cerr << "Ply: invalid value type (list) on line " << (line_num + 1) << std::endl;
            return false;
        }
    }
    ...
    {
        std::cerr << "Ply: element \"" << element.name() << "\" already has a property \"" << token
                  << '"' << (line_num + 1) << std::endl;
        return false;
    }
    ...
    if(type == PLY_TYPE_LIST)
        b = element.add_property(Property(token, count_type, value_type));
    else
...
void Ply::_find_native_format()
{
    ...
    if(ptr[0] == 0x12 && ptr[1] == 0x34 && ptr[2] == 0x56 && ptr[3] == 0x78)
        _native_format = PLY_FORMAT_BIN_BE;
    else if(ptr[0] == 0x78 && ptr[1] == 0x56 && ptr[2] == 0x34 && ptr[3] == 0x12)
