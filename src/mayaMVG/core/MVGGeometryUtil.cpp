...
void MVGGeometryUtil::cameraToView(M3dView& view, const MPoint& point, MPoint& viewPoint)
{
    ...
    viewPoint.y = round((newY + 0.5 + 0.5 * (view.portHeight() / (float)view.portWidth() - 1.0)) *
                        view.portWidth());
}
...
bool MVGGeometryUtil::projectFace2D(M3dView& view, MPointArray& face3DPoints,
                                    const MVGCamera& camera, const MPointArray& face2DPoints,
                                    MVector height)
{
    if(!camera.isValid())
        return false;
    ...
    if(height.length() != 0)
    {
        ...
        for(size_t i = 0; i < facePoints_view.size() - 2; ++i)
        { // remove extra point
    ...
    {
        for(size_t i = 0; i < facePoints_view.size() - 1; ++i)
        { // remove extra point
            worldPoint = viewToWorld(view, facePoints_view[i]);
            plane_line_intersect(model, cameraCenter, worldPoint, P);
            face3DPoints.append(P);
...
void MVGGeometryUtil::projectPointOnPlane(const MPoint& point, M3dView& view,
                                          PlaneKernel::Model& model, const MVGCamera& camera,
...
void MVGGeometryUtil::triangulatePoint(MPointArray& points2D, std::vector<MVGCamera>& cameras,
