...
: QObject(attachedView)
{
	...
	if(view == NULL)
	{
		LOG_ERROR("mlInstantCoding: attachedView must be a QDeclarativeView");
	}
...
void QmlInstantCoding::setWatching(bool watchValue)
{
	...
	if(_watching)
	{
		...
		QObject::connect(_fileWatcher, SIGNAL(fileChanged(QString)), this, SLOT(onFileChanged(QString)));
	}
	...
	// Disable the watcher
		...
		QObject::disconnect(_fileWatcher, SIGNAL(fileChanged(QString)), this, SLOT(onFileChanged(QString)));
	}
}
...
void QmlInstantCoding::addFilesFromDirectory(const QString& dirname, bool recursive)
{
	...
	if(!dirUrl.isValid())
	{
		LOG_ERROR("addFilesFromDirectory :  "<< dirname.toStdString()<< " is not a valid directory.");
		return;	
	}
