    ...
    switch(_manipUtil->getIntersectionState())
    {
        ...
        case MVGManipulatorUtil::eIntersectionPoint:
        {
            ...
            if((modifiers & Qt::ShiftModifier) || (modifiers & Qt::ControlModifier))
            {
                ...
                MIntArray connectedFaceIndex =
                    mesh.getConnectedFacesToVertex(intersectionData.pointIndex);
    ...
    switch(_moveState)
    {
        ...
        case eMovePoint:
        {
            if((modifiers & Qt::ShiftModifier) || (modifiers & Qt::ControlModifier))
            {
                ...
                _manipUtil->addUpdateFaceCommand(meshPath, _manipUtil->getPreviewFace3D(),
                                                 intersectionData.facePointIndexes);
        ...
        case eMoveEdge:
        {
            ...
            if((modifiers & Qt::ShiftModifier) || (modifiers & Qt::ControlModifier))
            {
                ...
                _manipUtil->addUpdateFaceCommand(meshPath, edgePoints,
                                                 intersectionData.edgePointIndexes);
                _manipUtil->getPreviewFace3D().clear();
                intersectionData.facePointIndexes.clear();
            }
            else
            {
                ...
                MDagPath meshPath;
                MVGMayaUtil::getDagPathByName(intersectionData.meshName.c_str(), meshPath);
                _manipUtil->addUpdateFaceCommand(meshPath, triangulatedPoints,
                                                 intersectionData.edgePointIndexes);
                intersectionData.facePointIndexes.clear();
            }
            break;
...
MStatus MVGMoveManipulator::doDrag(M3dView& view)
{
    ...
    MVGManipulatorUtil::IntersectionData& intersectionData = _manipUtil->getIntersectionData();
    std::vector<MVGManipulatorUtil::MVGPoint2D>& meshPoints =
        data->allPoints2D[intersectionData.meshName];
    ...
    switch(_moveState)
    {
        ...
        case eMovePoint:
        {
            ...
            {
                if(meshPoints[intersectionData.pointIndex].movableState >=
                   MVGManipulatorUtil::eMovableInSamePlane)
                    computeTmpFaceOnMovePoint(view, data, mousePoint);
            }
            else if(modifiers & Qt::ShiftModifier)
            {
                if(meshPoints[intersectionData.pointIndex].movableState ==
                   MVGManipulatorUtil::eMovableRecompute)
        ...
        {
            if(intersectionData.edgePointIndexes.length() == 0 ||
               meshPoints.size() < intersectionData.edgePointIndexes[0] ||
               meshPoints.size() < intersectionData.edgePointIndexes[1])
                break;
            if(modifiers & Qt::ControlModifier)
            {
                if(meshPoints[intersectionData.edgePointIndexes[0]].movableState >=
                       MVGManipulatorUtil::eMovableInSamePlane &&
                   meshPoints[intersectionData.edgePointIndexes[1]].movableState >=
                       MVGManipulatorUtil::eMovableInSamePlane)
...
void MVGMoveManipulator::drawIntersections(M3dView& view) const
{
    ...
    switch(_manipUtil->getIntersectionState())
    {
        case MVGManipulatorUtil::eIntersectionPoint:
        {
            if(intersectionData.pointIndex < 0 || meshPoints.size() < intersectionData.pointIndex)
                return;
            MVGManipulatorUtil::EPointState movableState =
                meshPoints[intersectionData.pointIndex].movableState;
            MPoint point =
                MVGGeometryUtil::worldToView(view, meshPoints[intersectionData.pointIndex].point3D);
            if(modifiers & Qt::ControlModifier)
            {
        ...
        case MVGManipulatorUtil::eIntersectionEdge:
        {
            ...
            MPoint viewPoint1 = MVGGeometryUtil::worldToView(
                view, meshPoints[intersectionData.edgePointIndexes[0]].point3D);
            MPoint viewPoint2 = MVGGeometryUtil::worldToView(
                view, meshPoints[intersectionData.edgePointIndexes[1]].point3D);
...
void MVGMoveManipulator::drawTriangulation(M3dView& view, MVGManipulatorUtil::DisplayData* data,
                                           const float mousex, const float mousey) const
{
    switch(_moveState)
    {
        case eMoveNone:
        ...
        case eMovePoint:
        {
            if(_manipUtil->getIntersectionData().pointIndex < 0)
                break;
            MPoint& point3D = data->allPoints2D[_manipUtil->getIntersectionData().meshName]
                                  .at(_manipUtil->getIntersectionData().pointIndex)
                                  .point3D;
        ...
        case eMoveEdge:
        {
            ...
            MPoint viewPoint1;
            MPoint viewPoint2;
            MPoint P1 = mousePoint +
                        _manipUtil->getIntersectionData().edgeRatio *
                            _manipUtil->getIntersectionData().edgeHeight2D;
            MPoint P0 = mousePoint -
                        (1 - _manipUtil->getIntersectionData().edgeRatio) *
                            _manipUtil->getIntersectionData().edgeHeight2D;
...
void MVGMoveManipulator::computeTmpFaceOnMovePoint(M3dView& view,
                                                   MVGManipulatorUtil::DisplayData* data,
                                                   const MPoint& mousePoint,
                                                   bool recomputePlane) const
{
    //
    MVGManipulatorUtil::IntersectionData& intersectionData = _manipUtil->getIntersectionData();
    std::vector<MVGManipulatorUtil::MVGPoint2D>& meshPoints2D =
        data->allPoints2D[intersectionData.meshName];
    ...
    if(recomputePlane)
    {
        ...
        for(int i = 0; i < faceVerticesId.length(); ++i)
        {
            ...
            {
                MPoint pCamera;
                MVGGeometryUtil::worldToCamera(view, meshPoints2D[faceVerticesId[i]].point3D,
                                               pCamera);
...
void MVGMoveManipulator::computeTmpFaceOnMoveEdge(M3dView& view,
                                                  MVGManipulatorUtil::DisplayData* data,
                                                  const MPoint& mousePoint, bool recompute) const
{
    MVGManipulatorUtil::IntersectionData& intersectionData = _manipUtil->getIntersectionData();
    std::vector<MVGManipulatorUtil::MVGPoint2D>& meshPoints2D =
        data->allPoints2D[intersectionData.meshName];
                                     ...
                                     MPoint& resultPoint3D) const
{
    ...
    MVGManipulatorUtil::DisplayData* complementaryData =
        _manipUtil->getComplementaryDisplayData(view);
    ...
    std::vector<MVGManipulatorUtil::MVGPoint2D>& complementaryMeshPoints2D =
        complementaryData->allPoints2D[intersectionData.meshName];
    if(intersectionData.pointIndex < 0 ||
       complementaryMeshPoints2D.size() < intersectionData.pointIndex)
        return false;
    ...
    MPoint pCamera;
    MVGGeometryUtil::worldToCamera(
        complementaryView, complementaryMeshPoints2D[intersectionData.pointIndex].point3D, pCamera);
...
bool MVGMoveManipulator::triangulateEdge(
    M3dView& view, const MVGManipulatorUtil::IntersectionData& intersectionData,
    const MPoint& mousePointInCameraCoord, MPointArray& resultPoint3D) const
{
    ...
    MPoint P1 =
        mousePointInCameraCoord + intersectionData.edgeRatio * intersectionData.edgeHeight2D;
    array1.append(P1);
    MPoint P0 =
        mousePointInCameraCoord - (1 - intersectionData.edgeRatio) * intersectionData.edgeHeight2D;
    array0.append(P0);
    ...
    std::vector<MVGManipulatorUtil::MVGPoint2D>& meshPoints2D =
        complementaryData->allPoints2D[intersectionData.meshName];
    if(intersectionData.edgePointIndexes.length() == 0 ||
       meshPoints2D.size() < intersectionData.edgePointIndexes[0] ||
       meshPoints2D.size() < intersectionData.edgePointIndexes[1])
        return false;
    MPoint pCamera0, pCamera1;
    MVGGeometryUtil::worldToCamera(
        complementaryView, meshPoints2D[intersectionData.edgePointIndexes[0]].point3D, pCamera0);
    MVGGeometryUtil::worldToCamera(
        complementaryView, meshPoints2D[intersectionData.edgePointIndexes[1]].point3D, pCamera1);
...
void MVGMoveManipulator::drawUI(MHWRender::MUIDrawManager& drawManager,
                                const MHWRender::MFrameContext&) const
{
