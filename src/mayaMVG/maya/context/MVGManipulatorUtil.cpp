...
bool MVGManipulatorUtil::intersectPoint(M3dView& view, DisplayData* displayData,
                                        const MPoint& pointInCameraCoordinates)
{
    if(!displayData)
        return false;
    ...
    for(std::map<std::string, std::vector<MVGPoint2D> >::iterator it = meshMap.begin();
        it != meshMap.end(); ++it)
    {
        ...
        for(int i = 0; i < meshPoints.size(); ++i)
        {
            ...
            if(p.x <= pointInCameraCoordinates.x + threshold &&
               p.x >= pointInCameraCoordinates.x - threshold &&
               p.y <= pointInCameraCoordinates.y + threshold &&
               p.y >= pointInCameraCoordinates.y - threshold)
            {
...
bool MVGManipulatorUtil::intersectEdge(M3dView& view, DisplayData* displayData,
                                       const MPoint& pointInCameraCoordinates)
{
    if(!displayData)
        return false;
    ...
    // Browse meshes
    for(std::map<std::string, std::vector<MIntArray> >::iterator it = _cacheMeshToEdgeArray.begin();
        it != _cacheMeshToEdgeArray.end(); ++it)
    {
        ...
        for(std::vector<MIntArray>::iterator edgeIt = edgesArray.begin();
            edgeIt != edgesArray.end(); ++edgeIt)
        {
            ...
            MVector AB = B - A;
            MVector AP = pointInCameraCoordinates - A;

            // Projected point on segment
            double AHLenght = (AB * AP) / AB.length();
            if(AHLenght > AB.length() || AP * AB < 0.)
                continue;

            distance = sqrt(pow(AP.length(), 2) - pow(AHLenght, 2));
...
void MVGManipulatorUtil::updateIntersectionState(M3dView& view, DisplayData* data,
                                                 const MPoint& mousePointInCameraCoordinates)
{
    if(data->allPoints2D.empty())
        return;
    resetIntersections();
    if(intersectPoint(view, data, mousePointInCameraCoordinates))
        _intersectionState = MVGManipulatorUtil::eIntersectionPoint;
    else if(intersectEdge(view, data, mousePointInCameraCoordinates))
        _intersectionState = MVGManipulatorUtil::eIntersectionEdge;
}

bool MVGManipulatorUtil::computeEdgeIntersectionData(M3dView& view, DisplayData* data,
                                                     const MPoint& mousePointInCameraCoord)
{
    std::vector<MVGPoint2D>& mvgPoints = data->allPoints2D[_intersectionData.meshName];
    if(_intersectionData.edgePointIndexes.length() == 0 ||
       mvgPoints.size() < _intersectionData.edgePointIndexes[0] ||
       mvgPoints.size() < _intersectionData.edgePointIndexes[1])
        return false;
    ...
    MIntArray connectedFaceIndex =
        mesh.getConnectedFacesToVertex(_intersectionData.edgePointIndexes[0]);
...
void MVGManipulatorUtil::rebuild()
{
    ...
    for(size_t i = 0; i < M3dView::numberOf3dViews(); ++i)
    {
        ...
        for(std::map<std::string, MPointArray>::iterator it = _cacheMeshToPointArray.begin();
            it != _cacheMeshToPointArray.end(); ++it)
        {
...
MVGManipulatorUtil::DisplayData* MVGManipulatorUtil::getDisplayData(M3dView& view)
{
    MDagPath path;
    view.getCamera(path);
    std::map<std::string, DisplayData>::iterator it =
        _cacheCameraToDisplayData.find(path.fullPathName().asChar());
...
MVGManipulatorUtil::DisplayData* MVGManipulatorUtil::getComplementaryDisplayData(M3dView& view)
{
    MDagPath path;
    view.getCamera(path);
    std::map<std::string, MVGManipulatorUtil::DisplayData>::iterator it =
        _cacheCameraToDisplayData.begin();
...
void MVGManipulatorUtil::resetTemporaryData()
{
    ...
    for(std::map<std::string, DisplayData>::iterator it = _cacheCameraToDisplayData.begin();
        it != _cacheCameraToDisplayData.end(); ++it)
...
bool MVGManipulatorUtil::addCreateFaceCommand(const MDagPath& meshPath,
                                              const MPointArray& facePoints3D)
{
    // Undo/redo
...
bool MVGManipulatorUtil::addUpdateFaceCommand(const MDagPath& meshPath,
                                              const MPointArray& newFacePoints3D,
