...
MStatus MVGCreateManipulator::doPress(M3dView& view)
{
    ...
    {
        MVGManipulatorUtil::DisplayData* complementaryCache =
...
MStatus MVGCreateManipulator::doRelease(M3dView& view)
{
    ...
    switch(_createState)
    {
        ...
        {
            MDagPath meshPath;
            MVGMayaUtil::getDagPathByName(_manipUtil->getIntersectionData().meshName.c_str(),
                                          meshPath);
...
void MVGCreateManipulator::drawUI(MHWRender::MUIDrawManager& drawManager,
        ...
        case MVGManipulatorUtil::eIntersectionEdge:
        {
            ...
            if(intersectionData.edgePointIndexes.length() == 0 ||
               ...
               meshPoints.size() < intersectionData.edgePointIndexes[1])
                return;
            MPoint A = MVGGeometryUtil::worldToView(
                view, meshPoints[intersectionData.edgePointIndexes[0]].point3D);
            MPoint B = MVGGeometryUtil::worldToView(
...
void MVGCreateManipulator::drawOtherPreview2D(M3dView& view,
                                              const MVGManipulatorUtil::DisplayData* data)
{
    if(!data)
        return;
...
void MVGCreateManipulator::computeTmpFaceOnEdgeExtend(M3dView& view,
                                                      MVGManipulatorUtil::DisplayData* data,
                                                      const MPoint& mousePointInCameraCoord)
{
    MVGManipulatorUtil::IntersectionData& intersectionData = _manipUtil->getIntersectionData();
    std::vector<MVGManipulatorUtil::MVGPoint2D>& mvgPoints =
        data->allPoints2D[intersectionData.meshName];

    // Get edge 3D points
    if(intersectionData.edgePointIndexes.length() == 0 ||
       mvgPoints.size() < intersectionData.edgePointIndexes[0] ||
       mvgPoints.size() < intersectionData.edgePointIndexes[1])
        return;
    ...
    MVGGeometryUtil::worldToCamera(view, mvgPoints[intersectionData.edgePointIndexes[0]].point3D,
                                   pCamera0);
    MVGGeometryUtil::worldToCamera(view, mvgPoints[intersectionData.edgePointIndexes[1]].point3D,
                                   pCamera1);
    ...
    // Compute 3D face
    _manipUtil->getPreviewFace3D().clear();
    if(!MVGGeometryUtil::projectFace2D(view, _manipUtil->getPreviewFace3D(), data->camera,
                                       previewPoints2D, intersectionData.edgeHeight3D))
    {
