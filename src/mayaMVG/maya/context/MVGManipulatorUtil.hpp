...
public:
    ...
    struct IntersectionData
    {
        ...
        MIntArray facePointIndexes; // Indexes of the points of the first face connected to
                                    // intersected point or edge
    };
    ...
    struct DisplayData
    {
        ...
        MPointArray buildPoints2D; // Temporary points in Camera before having 3D
                                   // information
...
public:
    ...
    void updateIntersectionState(M3dView& view, DisplayData* data,
                                 const MPoint& mousePointInCameraCoordinates);
    bool computeEdgeIntersectionData(M3dView& view, DisplayData* data,
    ...
    bool addUpdateFaceCommand(const MDagPath& meshPath, const MPointArray& newFacePoints3D,
...
private:
    ...
    std::map<std::string, MPointArray>
        _cacheMeshToPointArray; // Map from meshName to mesh points (Temporary)
    std::map<std::string, std::vector<EPointState> >
        _cacheMeshToMovablePoint; // Map from meshName to numConnectedFace by
                                  // point (Temporary)
    std::map<std::string, std::vector<MIntArray> >
        _cacheMeshToEdgeArray; // Map from meshName to edge points ID (Temporary)
};
